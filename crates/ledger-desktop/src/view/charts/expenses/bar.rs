            ...
            |this, cx| {
                ...
                this.inner.update(cx, |this, cx| {
                    ...
                    let values = values
                        ...
                        .collect();
                    this.refresh_data(&bars, values, None, false, cx);
                });
                cx.notify();
            },
        ));
...
/// Aggregate daily expense data by month (for Year period)
fn aggregate_by_month(
    ...
    let mut months: Vec<(u32, i32, NaiveDate, NaiveDate)> = vec![]; // (month, year, start, end)
    ...
    // Create bar data and aggregate values
    ...
    for (month, _year, start_date, end_date) in &months {
        ...
        // Sum daily balances for this month
...
/// Aggregate daily expense data by week (for Month period)
fn aggregate_by_week(
    ...
    // Group dates by week (7-day chunks from the start of the period)
    ...
    for (start_date, end_date) in &weeks {
        ...
        // Sum daily balances for this week
...
/// Aggregate daily expense data by day (for Week period)
fn aggregate_by_day(
    daily_balance: &DailyBalance,
    range: std::ops::Range<NaiveDate>,
) -> (Vec<bar::BarData>, HashMap<String, Vec<Option<f64>>>) {
    ...
    let mut current_date = range.start;
    while range.contains(&current_date) {
        // Use day name as label
        let label = current_date.format("%a").to_string(); // Mon, Tue, etc.
        ...
        // Sum daily balances for this day
